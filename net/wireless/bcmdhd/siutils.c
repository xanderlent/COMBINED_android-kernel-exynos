	...
	if (!si_onetimeinit) {
		...
		if (!FWSIGN_ENAB()) {
			...
			if (BCM4369_CHIP(sih->chip) || BCM4362_CHIP(sih->chip) ||
				BCM4368_CHIP(sih->chip) || BCM4389_CHIP(sih->chip) ||
...
#ifdef BCMINTERNAL
	/* For 8 byte access, address has to be 8 byte aligned */
	if (size == 8) {
		if (addr & 0x7) {
			bcmerror = BCME_ERROR;
		}
...
si_backplane_access_64(si_t *sih, uint addr, uint size, uint64 *val, bool read)
{
	...
	if (CHIPTYPE(sih->socitype) == SOCI_NCI) {
		return nci_backplane_access_64(sih, addr, size, val, read);
	}
...
{
#if defined(BCMINTERNAL) || defined(BCMDBG_ERR) || defined(BCMASSERT_SUPPORT) || \
	defined(BCMDBG_DUMP)
	if (CHIPTYPE(sih->socitype) == SOCI_SB)
...
#else
	return FALSE;
#endif /* BCMINTERNAL || BCMDBG_ERR || BCMASSERT_SUPPORT || BCMDBG_DUMP */
}
...
si_pciesbreg(si_t *sih, uint32 offset, uint32 mask, uint32 val)
{
	...
	if (!PCIE_GEN1(sii)) {
		SI_ERROR(("si_pciesbreg: Not a PCIE device\n"));
		return 0;
	}
