...
#define WL_RSPEC_BW_320MHZ	0x00050000
...
/* check if rate field is legacy rate */
#define RSPEC_ISRATE(rspec)	RSPEC_ISLEGACY(rspec)
/* check if rate field is HT mcs */
#define RSPEC_ISMCS(rspec)	RSPEC_ISHT(rspec)
/* fast check if rate field is NSS+MCS (vs other formats) */
#define RSPEC_ISNSSMCS(rspec)	(RSPEC_ENCODE(rspec) >= \
				 (WL_RSPEC_ENCODE_VHT >> WL_RSPEC_ENCODING_SHIFT))
