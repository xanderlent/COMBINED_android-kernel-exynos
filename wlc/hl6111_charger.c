...
static int hl6111_get_random_num(void)
{
    unsigned int rand;

    get_random_bytes(&rand, sizeof(rand));
    rand %= RANDOM_NUM_LIMIT;
    pr_info("random Num:[0x%x]\n", rand);
    return rand;
}

static void hl6111_rx_send_auth_packet(struct hl6111_charger *chg)
{
    /****************************************************/
    /*    Reg0x43: B1,                                  */
    /*    Reg0x42: B2                                   */
    /*    Reg0x41: B3                                   */
    /*    Reg0x40: C1                                   */
    /*    Reg0xE5: ID_B4   -->   value of REG0xE5       */
    /*    Reg0xE6: ID_B5   -->   value of REG0xE6       */
    /****************************************************/
    /*    B1: 0x10                                      */
    /*    B2, B3: random number generated by AP         */
    /*    k1: 0x35                                      */
    /*    K2: 0x86                                      */
    /*    ID_B4, ID_B5: B4, B5 of ID packet             */
    /*    C1: LSB of Calculated authentication C        */
    /****************************************************/
    /*    The steps to send PPP by AP:                  */
    /*      1.  Enable password                         */
    /*      2.  Reg0x71=0x80                            */
    /*      3.  Reg0x43=0x10                            */
    /*      4.  Reg0x42,41=random number                */
    /*      5.  Reg0x40: authentication result          */
    /*      6.  Reg0x44: send PPP                       */
    /****************************************************/
    /*   B = (B2<<8) + B3;                              */
    /*   C = ((B^K1) << 5) + (B>>8)^K2) + ID_B4 + ID_B5;*/
    /*   C1 = Takes a byte of C(LSB)                    */
    /****************************************************/

    unsigned int ran_b2, ran_b3, id_b4, id_b5;
    unsigned int b, c, c1, k1, k2;

    LOG_DBG("start sending!\n");
    k1 = 0x35;
    k2 = 0x86;

    ran_b2 = hl6111_get_random_num();
    ran_b3 = hl6111_get_random_num();

    hl6111_read_reg(chg, 0xE5, &id_b4);
    hl6111_read_reg(chg, 0xE6, &id_b5);

    b = (ran_b2 << 8) + ran_b3;
    c = ((b ^ k1) << 5) + ((b >> 8) ^k2) + id_b4 + id_b5;
    c1 = 0xFF & c;

    LOG_DBG("c[0x%x], c1[0x%x], ran_b2[0x%x], ran_b3[0x%x]\n", c, (u8)c1, (u8)ran_b2, (u8)ran_b3);

    /* Authentication code */
    hl6111_write_reg(chg, 0x71, 0x00);
    hl6111_write_reg(chg, 0x43, 0x10);
    hl6111_write_reg(chg, 0x42, (u8)ran_b2);
    hl6111_write_reg(chg, 0x41, (u8)ran_b3);
    hl6111_write_reg(chg, 0x40, (u8)c1);

    /* send ppp */
    hl6111_write_reg(chg, 0x44, 0x88);

    /* enable pasword*/
    hl6111_write_reg(chg, 0xD9, 0x9F);
    hl6111_write_reg(chg, 0xDA, 0xFE);

    /* */
    hl6111_write_reg(chg, 0x71, 0x80);
}
...
static int hl6111_parse_dt(struct device *dev, struct hl6111_platform_data *pdata)
{
    ...
    pdata->auth_gpio = of_get_named_gpio(np, "halo,auth-gpio", 0);
    LOG_DBG("irq-auth:: %d \r\n", pdata->det_gpio);
...
static irqreturn_t hl6111_interrupt_handler(int irg, void *data)
{
    ...
    int ret;
    if (chg->pdata->auth_gpio >= 0 && !chg->tx_authenticated) {
        return IRQ_HANDLED;
    }
...
static void hl6111_auth_irq_set_state(struct hl6111_charger *chg, int irq_enable) {
    dev_info(chg->dev, "%s: irq_enable: %d, irq_state: %d\n",
            __func__, irq_enable, chg->irq_state);
    if (irq_enable) {
        if (--chg->irq_state == 0)
            enable_irq(chg->pdata->irq_auth);
        else
            dev_err(chg->dev, "%s: auth_irq issue: umatched enables\n", __func__);
    } else {
        if (chg->irq_state > 0) {
            dev_err(chg->dev, "%s: auth_irq issue: multiple disable\n", __func__);
            return;
        }
        if (chg->irq_state == 0) {
            chg->irq_state++;
            disable_irq(chg->pdata->irq_auth);
        }
    }
}

static void hl6111_charge_power_detected(struct hl6111_charger *chg) {
    unsigned long current_time = ktime_get_boot_ns() / 1000000000;
    ...
    if (chg->online) {
        ...
        if (chg->pdata->auth_gpio >= 0 && !chg->tx_authenticated) {
            if (chg->timer_id == TIMER_ID_NONE) {
                ...
                hl6111_auth_irq_set_state(chg, 1);
            } else {
                LOG_DBG("%s: Got power attached during authentication\n", __func__);
            }
        } else {
            if (chg->tx_authenticated) {
                dev_dbg(chg->dev, "%s: TX already authenticated, not rerunning\n", __func__);
            }
        ...
        chg->last_off_time = current_time;
        if (chg->pdata->auth_gpio >= 0) {
            if (chg->timer_id == TIMER_START_AUTH || chg->timer_id == TIMER_DETECT_LOW || chg->timer_id == TIMER_END_AUTH) {
                LOG_DBG("%s: Lost TX power as expected during authentication\n", __func__);
                return;
            }
            if (chg->timer_id == TIMER_PRESET_AUTH) {
                dev_info(chg->dev, "%s: Lost TX power unexpectedly. Cancel authentication\n", __func__);
                cancel_delayed_work(&chg->auth_work);
                                        ...
                                        const union power_supply_propval *val)
{
    ...
    switch(psp) {
        case POWER_SUPPLY_PROP_ONLINE:
            ...
            hl6111_charge_power_detected(chg);
            break;
...
static irqreturn_t hl6111_auth_irq_handler(int irq, void *data)
{
    struct hl6111_charger *chg = data;
    int sts;

    LOG_DBG("%s: irq detected\n", __func__);
    if (!chg->tx_det || chg->pdata->auth_gpio < 0) {
        return IRQ_HANDLED;
    }
    sts = gpio_get_value(chg->pdata->auth_gpio);
    if (sts == 0 && !chg->auth_chok) {
        chg->auth_chok = true;
        if (chg->timer_id == TIMER_ID_NONE || chg->timer_id == TIMER_PRESET_AUTH) {
            dev_info(chg->dev, "%s: Start timer for Authentication!\n", __func__);
            chg->timer_id = TIMER_PRESET_AUTH;
        ...
        chg->auth_chok = false;
        if (chg->timer_id == TIMER_PRESET_AUTH) {
            dev_info(chg->dev, "%s: TX disconnected during auth setup\n", __func__);
            cancel_delayed_work(&chg->auth_work);

        } else if (chg->timer_id == TIMER_START_AUTH) {
            chg->timer_id = TIMER_DETECT_LOW;

            LOG_DBG("%s: Detect LOW, while Queue is working!\n", __func__);
            //Don't need to start workqueue again, because timer is already started.
        }
    }
...
static int hl6111_irq_auth_init(struct hl6111_charger *chg) {
    struct hl6111_platform_data *pdata = chg->pdata;
    int ret, irq_auth;

    if (pdata->auth_gpio >= 0) {
        irq_auth = gpio_to_irq(pdata->auth_gpio);

        ret = request_threaded_irq(irq_auth, NULL, hl6111_auth_irq_handler,
                              IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
                              "wireless-auth-irq", chg);
        if(ret < 0)
            goto fail_irq_det;
        pdata->irq_auth = irq_auth;
        hl6111_auth_irq_set_state(chg, 0);

        LOG_DBG("irq_auth::%d, auth_gpio::%d\r\n",pdata->irq_auth,  pdata->auth_gpio);
    }
    return 0;
...
static void hl6111_rx_authentication_work(struct work_struct *work)
{
    struct hl6111_charger *chg = container_of(work, struct hl6111_charger, auth_work.work);

    switch (chg->timer_id) {
        case TIMER_PRESET_AUTH:
            /*Send authentication packet!*/
            dev_info(chg->dev, "PRESET Start!\n");
            hl6111_rx_send_auth_packet(chg);

            chg->timer_id = TIMER_START_AUTH;
            schedule_delayed_work(&chg->auth_work, msecs_to_jiffies(1000));
            break;

        case TIMER_START_AUTH:
            if (chg->auth_retry < 3) {
                LOG_DBG("Need to retry! retry-cnt[%d]\n", chg->auth_retry);
                hl6111_rx_send_auth_packet(chg);
                schedule_delayed_work(&chg->auth_work, msecs_to_jiffies(1000));
                chg->auth_retry++;
            } else {
                dev_info(chg->dev, "Non-Google TX connected!\n");
                chg->auth_retry = 0;
                chg->tx_authenticated = false;
                chg->tx_valid = false;
        ...
        case TIMER_END_AUTH:
            dev_info(chg->dev, "Google Tx connected!\n");
        ...
        case TIMER_DETECT_LOW:
            dev_info(chg->dev, " Detect pin didn't go high for 1 sec, it's real off\n");
...
static int hl6111_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   if (pdata->int_gpio >= 0){
    ...
    if (pdata->auth_gpio < 0) {
        dev_info(&client->dev, "No authentication GPIO, authentication not enabled\n");
        // Assume authenticity
    } else {
        device_init_wakeup(charger->dev, true);
        INIT_DELAYED_WORK(&charger->auth_work, hl6111_rx_authentication_work);
        charger->auth_retry = 0;
        charger->timer_id = TIMER_ID_NONE;
        ret = hl6111_irq_auth_init(charger);
    }
...
static int hl6111_charger_remove(struct i2c_client *client)
{
    ...
    if (charger->pdata->auth_gpio >= 0) {
        gpio_free(charger->pdata->auth_gpio);
    }
