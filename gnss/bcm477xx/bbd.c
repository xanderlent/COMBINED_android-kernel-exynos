 ...
 */
static struct bbd_device bbd; /* TODO remove global variable */
/*
 ...
 */
void bbd_register(void *priv, struct bbd_callbacks *cb)
{
	bbd.ssp_priv = priv;
	bbd.ssp_cb = cb;
}
...
struct sensor_pkt {
	...
	/*We assume max SSP packet less than 1KB */
 ...
 * @size:
 * @timeout_ms: if specified, this function waits for
 * sensor packet during given time
 *
 * @return: popped data length = success
 */
ssize_t bbd_pull_packet(
		unsigned char *buf, size_t size, unsigned int timeout_ms)
{
	 ...
	 * Because SHMD's buffer is linear,
	 * we may require 2 copies from [tail..end] and [start..head]
	 */
	do {
		...
		memcpy(buf + rd_size, circ->buf + circ->tail, copied);
		size -= copied;
		rd_size += copied;
 ...
 *
 *      BBD pushes reset request into /dev/bbd_control and actual
 *      reset is done by gpsd/lhd when it reads the request
 *
 ...
 *
 ************************/
static ssize_t store_sysfs_bbd_control(
...
static ssize_t show_sysfs_bbd_pl(
...
static DEVICE_ATTR(bbd, 0644, NULL, store_sysfs_bbd_control);
static DEVICE_ATTR(pl, 0644, show_sysfs_bbd_pl, NULL);
		...
		unsigned long ulDataLen)
{
	...
	print_hex_dump(KERN_INFO, pIntroduction, DUMP_PREFIX_NONE, 32, 1,
			pData, ulDataLen, false);
}
...
int bbd_init(struct device *dev, bool legacy_patch)
{
	...
	for (minor = 0; minor < BBD_DEVICE_INDEX; minor++) {
		...
		if (minor == BBD_MINOR_SHMD)
			continue;
		/* Reserve char device number
		 * (a.k.a, major, minor) for this BBD device
		 */
		ret = register_chrdev_region(devno, 1, name);
		if (ret) {
			pr_err("failed to register_chrdev_region() \"%s\", ret=%d",
					name, ret);
			goto free_class;
		}
	...
	if (ret < 0) {
		pr_err("failed to sysfs_create_group \"bbd\", ret = %d",
				 ret);
		goto free_kobj;
	}
