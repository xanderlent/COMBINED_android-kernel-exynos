...
static int bbd_suspend(pm_message_t state)
{
	 ...
	 * the chips sensor hub system. Moving the delay to within the define
	 * removes it when the features is not used. Updating to a non-blocking
	 * sleep instead of a busy wait.
	 */
	msleep(20);
#endif
