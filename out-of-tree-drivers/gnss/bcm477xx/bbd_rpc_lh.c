...
/* Defines the max size of incoming packet.
 * That should match the max size of outgoing packet from the remote TL
 */
...
/* Indicates that this is a reliable packet.
 * Flag detail contains the remote reliable seqId
 */
...
/* Size of packet will have one byte extension (MSB),
 * contained in the Flags detail
 */
static const unsigned short FLAG_EXTENDED        = (1<<7);
/* If set, then the flag details will contains a Byte
 * representing the MSB of the 16bit flags
 */
...
struct stTransportLayerStats {
	...
	/* this is approximate as it is reported and
	 * the report could be lost.
	 */
...
static unsigned char m_aucRxMessageBuf[
	MAX_INCOMING_PACKET_SIZE+MAX_HEADER_SIZE];
static unsigned char m_aucRxEscapedBuf[
	(MAX_INCOMING_PACKET_SIZE+MAX_HEADER_SIZE+2)*2];
static struct stTransportLayerStats m_otCurrentStats;
...
static inline unsigned char crc_calc(
	unsigned char *m_ucCrcState, unsigned char ucData)
 ...
 *
 */
static unsigned char crc_calc_many(
	unsigned char *m_ucCrcState, const unsigned char *pucData,
 ...
 *
 */
static int BbdBridge_OnRpcReceived(
	unsigned short usRpcId, unsigned char *pRpcPayload,
	unsigned short usRpcLen)
{
	if (usRpcId == RPC_DEFINITION(IRpcSensorResponse, Data)) {
		...
		if (size != (usRpcLen - 2) || size != (short) result)
			pr_err("length wrong, size=%d, usRpcLen=%d, result=%ld",
					size, usRpcLen, result);
 ...
 */
static bool BbdBridge_CheckPacketSanity(
 ...
 *
 */
static int BbdBridge_OnPacketReceived(
	unsigned char *pucData, unsigned short usSize)
{
	int sensor = 0, gnss = 0;
...
#ifdef DEBUG_1HZ_STAT
			bbd_update_stat(
				STAT_RX_RPC, usRpcLen+pucData-pucDataOrg);
#endif
			pucData += usRpcLen;
			lSize -= usRpcLen;
		}
	} else {
		pr_err("check packet sanity fail");
	}
	return (sensor > 0);
}

bool TransportLayer_PayloadReceived(void *priv, unsigned char ucCrc)
{
	...
	unsigned char ucExpectedTxSeqId;
	...
	for (i = 0; i < 16 && usFlags != 0 && usLen > 0; ++i) {
		...
		if (usFlagBit == FLAG_PACKET_ACK) /* acknowledgment */{
			...
			/* This is a reliable packet.
			 * we need to provide the proper Ack
			 */
			...
			m_otCurrentStats.ulRemoteGarbage +=
				ucFlagDetail;
	 ...
	 */
	ucExpectedTxSeqId = (m_ucLastRxSeqId + 1) & 0xFF;
	...
	bProcessPacket = bReliablePacket || usLen > 0;
	if (bReliablePacket) {
		 ...
		 * we didn't received it before!  if we did, the Host probably
		 * didn't received the Ack, so let's just send the ack
		...
		} else if (ucReliableSeqId ==
				((m_ucReliableSeqId + 1) & 0xFF)) {
			/* this is a valid message, just do nothing but update
			 * the reliable info.
			 * the message will be processed below
			 */
			usAckFlags |= FLAG_PACKET_ACK | FLAG_RELIABLE_ACK;
	...
	if (bProcessPacket) {
		if (bInternalPacket) {
			bcm_on_packet_received(priv,
				m_aucRxEscapedBuf, m_uiEscLen);
		} else {
			/* everything good, notify upper layer that we have
			 * the payload of a packet available In case
			 * of sensor data, we don't want send it to lhd!
			 */
...
static bool TransportLayer_PacketReceived(void *priv)
{
	...
	return TransportLayer_PayloadReceived(priv, ucCrc);
}
	...
	while (usIdx != usLen) {
		...
		case WAIT_FOR_ESC_SOP:
			if (ucData == ESCAPE_CHARACTER) {
				...
				/* if we had only one byte,
				 * then there is no garbage,
				 * any extra is garbage
				 */
				m_ulByteCntSinceLastValidPacket = 1;
			}
			break;
...
unsigned char sample1[] = { 0x00, 0x2E, 0xB0, 0x00, 0x85, 0x26, 0x00, 0x21,
	0x24, 0x22, 0x00, 0x02, 0x00, 0x1E, 0x00, 0x37, 0x0E, 0xD3, 0x00, 0xCB,
	0xFF, 0x50, 0x00, 0xDE, 0x00, 0x56, 0x00, 0xC7, 0xFF, 0x02, 0x00, 0x00,
	0x00, 0x37, 0x00, 0x90, 0xFE, 0x18, 0xFF, 0xB7, 0x46, 0x02, 0x00, 0x00,
	0x00, 0xB2, 0xB0, 0x01 };
