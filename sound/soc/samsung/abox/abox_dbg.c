		...
		struct device_attribute *attr, char *buf)
{
	...
	buf_len += sprintf(&buf[buf_len], "A-Box CPU register dump (%c%c%c%c)\n",
			version[3], version[2], version[1], version[0]);
	buf_len += sprintf(&buf[buf_len], "----------------------------------------\n");
	for (i = 0; i <= 14; i++) {
		buf_len += sprintf(&buf[buf_len], "CA7_R%02d        : %08x\n", i,
				readl(data->sfr_base + ABOX_CPU_R(i)));
	}
	buf_len += sprintf(&buf[buf_len], "CA7_PC         : %08x\n",
			readl(data->sfr_base + ABOX_CPU_PC));
	buf_len += sprintf(&buf[buf_len], "CA7_L2C_STATUS : %08x\n",
			readl(data->sfr_base + ABOX_CPU_L2C_STATUS));
	buf_len += sprintf(&buf[buf_len], "========================================\n");
