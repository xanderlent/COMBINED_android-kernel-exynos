...
static const struct snd_kcontrol_new snd_controls[] = {
	...
	SOC_SINGLE_EXT("DMIC_MCU On", SND_SOC_NOPM, 0, 1, 0,
...
{
	int ret;
	// Variable to store whether mcu dmic is on or off.
	// 1: it is on, 0: it is off.
	struct mcu_mic_codec_data *codec_data;

	dev_info(&pdev->dev, "Called mcu_mic_probe\n");

	codec_data = devm_kzalloc(
		&pdev->dev, sizeof(struct mcu_mic_codec_data), GFP_KERNEL);
	if (!codec_data) {
	...
	ret = snd_soc_register_codec(
		&pdev->dev, &mcu_mic_soc_codec_driver, &mcu_mic_soc_dai_driver, 1);
