...
unsigned char raydium_read_flash_log(struct raydium_ts_data *ts)
{
	unsigned char u8_buf[4];
	...
	if (u8_logcount != 0) {
		...
		raydium_read_fpc_flash(ts->client, u32_temp, u8_buf);
		pr_info("[touch]Rad log fw version 0x%x.0x%x.0x%x.0x%x\n",
			u8_buf[0], u8_buf[1], u8_buf[3], u8_buf[2]);
		if ((ts->id  & 0x1000) != 0)
			ts->id = 0x1000 | ((u8_buf[0] & 0xF) << 8) | u8_buf[1];
		else if ((ts->id  & 0x2000) != 0)
			ts->id = 0x2000 | ((u8_buf[0] & 0xF) << 8) | u8_buf[1];
...
unsigned char raydium_mem_table_setting(struct raydium_ts_data *ts)
{
	unsigned char u8_ret = SUCCESS;
#ifdef ENABLE_FLASHLOG_BACKUP
	unsigned char u8_buf[4];
	static unsigned char u8_readflash = false;
#endif
	...
	case RAD_21:
		...
		break;
	default:
		pr_info("[touch]mapping ic setting fail\n");
#ifdef ENABLE_FLASHLOG_BACKUP
		if (!u8_readflash) {
			u8_ret = raydium_read_flash_log(ts);
			u8_readflash = true;

			raydium_i2c_pda_read(ts->client,
					RAD_PDA2_CTRL_CMD,
					u8_buf,
					4);
			u8_buf[0] |= RAD_ENABLE_PDA2 | RAD_ENABLE_SI2;
			raydium_i2c_pda_write(ts->client,
					RAD_PDA2_CTRL_CMD,
					u8_buf,
					4);
			...
			if ((ts->id & 0x1000) != 0) {
				...
				memcpy(g_rad_para_image,
					...
					RAD_PARA_1X_SIZE);
				memcpy(g_rad_testfw_image,
					u8_rad_testfw_10 + 0xA00,
					RAD_TESTFW_1X_SIZE);
				memcpy(g_rad_testpara_image,
					u8_rad_testfw_10 + 0xF1C,
					RAD_PARA_1X_SIZE);
				ts->id = RAD_10;
			} else if ((ts->id & 0x2000) != 0) {
				memcpy(g_rad_boot_image,
...
unsigned char raydium_id_init(struct raydium_ts_data *ts,
			      ...
			      unsigned char u8_check_crc)
{
	...
	unsigned char u8_bl_version[4];
	struct raydium_ts_data *ts =
		(struct raydium_ts_data *)i2c_get_clientdata(client);

	/*##### wait for boot-loader start #####*/
	i32_ret = raydium_i2c_pda_read(client, 0x00000080, u8_bl_version, 4);
	if (i32_ret < 0)
		goto exit_upgrade;
	...
	*/
	...
	msleep(500);
	if (u8_bl_version[2] >= 0x0B) {
		do {
			raydium_i2c_pda2_read(client, 0x00, u8_buf, 1);
			usleep_range(4500, 5500);
			u16_retry--;
		} while ((u8_buf[0] != 0x55) && (u16_retry != 0));

		if (u8_buf[0] != 0x55) {
			pr_err("[touch]Error, wait erase/wrt finish\n");
			pr_err("u8_buf[0] : 0x%x\r\n", u8_buf[0]);
			i32_ret = ERROR;
			goto exit_upgrade;
		}
	}
			      ...
			      unsigned char u8_check_crc)
{
	...
	if (u8_buf[2] >= 4) {
		...
		/*#confirm in burn mode*/
		if (wait_fw_state(client, 0x20000214, 255, 2000, u16_retry) == ERROR) {
			pr_err("[touch]Error, confirm in burn mode\n");
			i32_ret = ERROR;
			goto exit_upgrade;
		}
			...
			unsigned short u16_i2c_data)
{
	...
	if (raydium_mem_table_setting(ts)) {
...
int raydium_load_test_fw(struct i2c_client *client)
{
	...
	if (ts->id == RAD_20 || ts->id == RAD_21) {
		...
		if (u8_buf[2] > 2) {
			pr_err("[touch]bootloader version %d,!!\n", u8_buf[1]);
			memset(u8_buf, 0, sizeof(u8_buf));
