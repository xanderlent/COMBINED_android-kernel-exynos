...
typedef enum mp_criteria_item {
	...
	AIN_Y_E,
	AIN_KEY_E,
	PS_Config_Lmt_Short_Rawdata_P_E,
	PS_Config_Lmt_Short_Rawdata_N_E,
	PS_Config_Lmt_Open_Rawdata_P_E,
	PS_Config_Lmt_Open_Rawdata_N_E,
	PS_Config_Lmt_FW_Rawdata_P_E,
	PS_Config_Lmt_FW_Rawdata_N_E,
	PS_Config_Lmt_FW_CC_P_E,
	PS_Config_Lmt_FW_CC_N_E,
	PS_Config_Lmt_FW_Diff_P_E,
	PS_Config_Lmt_FW_Diff_N_E,
	PS_Config_Diff_Test_Frame_E,
	MP_SETTIGN_CRITERIA_ITEM_LAST_E
} mp_criteria_item_e;
...
int32_t parse_mp_setting_ain_array(char **ptr, const char *item_string, uint8_t *ain_array, uint32_t IC_CFG_SIZE) {
    ...
    *ptr = tmp;
    ...
    while ((token = strsep(&tok_ptr, ", \t\r\0"))) {
        ...
        //printk("%3d, ", ain_array[i]);
        snprintf(print_buf + 5 * i, sizeof(print_buf), "%3d, ", ain_array[i]);
    ...
    if (i != IC_CFG_SIZE) {
        NVT_ERR("%s: load AIN_X failed!, i=%d, IC_CFG_SIZE=%d\n", __func__, i, IC_CFG_SIZE);
        return -1;
    }
    // go forward
    *ptr = *ptr + offset;
...
int32_t parse_mp_criteria_item_array(char **ptr, const char *item_string, int32_t *item_array, int32_t x_size) {
    char *tmp = NULL;
    int32_t i = 0;
    int32_t j = 0;
    char *token = NULL;
    ...
    *ptr = tmp;
    ...
    for (i = 0; i < Y_Channel; i++) {
        ...
        while ((token = strsep(&tok_ptr, ", \t\r\0"))) {
            if (strlen(token) == 0)
                continue;
            item_array[i * x_size + j] = (int32_t) simple_strtol(token, NULL, 10);
            //printk("%5d, ", item_array[i * X_Channel + j]);
            snprintf(print_buf + 7 * j, sizeof(print_buf), "%5d, ", item_array[i * x_size + j]);
            j++;
        }
        //printk("\n");
        NVT_LOG("%s\n", print_buf);
        // check if j equals to x_size
        if (j != x_size) {
            NVT_ERR("%s: j not equal X size!, j=%d, x_size=%d\n", __func__, j, x_size);
            return -1;
        }
        // go forward
        *ptr = *ptr + offset;
    ...
    while ((token = strsep(&tok_ptr, ", \t\r\0"))) {
        if (strlen(token) == 0)
            continue;
        item_array[Y_Channel * x_size + i] = (int32_t) simple_strtol(token, NULL, 10);
        //printk("%5d, ", item_array[Y_Channel * X_Channel + i]);
        snprintf(print_buf + 7 * i, sizeof(print_buf), "%5d, ", item_array[Y_Channel * x_size + i]);
        i++;
    }
    //printk("\n");
    NVT_LOG("%s\n", print_buf);
    ...
    if (i != Key_Channel) {
        NVT_ERR("%s: i not equal Key_Channel!, i=%d, Key_Channel=%d\n", __func__, i, Key_Channel);
        return -1;
    }
    // go forward
    *ptr = *ptr + offset;
...
int32_t nvt_load_mp_criteria(void){
	...
	int32_t read_ret = 0;
	char *ptr = NULL;
	mp_criteria_item_e mp_criteria_item = IC_X_CFG_SIZE_E;
	...
	retval = vfs_stat(mpcriteria, &stat);
	if (!retval) {
		fbufp = (char *)kzalloc(stat.size + 2, GFP_KERNEL);
		if (!fbufp) {
		...
		if (read_ret > 0) {
			...
			while ( ptr && (ptr < (fbufp + stat.size))) {
				if(mp_criteria_item == IC_X_CFG_SIZE_E) {
					ptr = fbufp;
                    if (parse_mp_setting_criteria_item(&ptr, "IC_X_CFG_SIZE:", (int32_t *) &IC_X_CFG_SIZE) < 0) {
                        retval = -1;
                        goto exit_free;
                    }
				} else if(mp_criteria_item == IC_Y_CFG_SIZE_E){
					ptr = fbufp;
                	if (parse_mp_setting_criteria_item(&ptr, "IC_Y_CFG_SIZE:", (int32_t *) &IC_Y_CFG_SIZE) < 0) {
                        retval = -1;
                        goto exit_free;
                    }
...
#if TOUCH_KEY_NUM > 0
				else if(mp_criteria_item == IC_KEY_CFG_SIZE_E) {
                    ptr = fbufp;
                    if (parse_mp_setting_criteria_item(&ptr, "IC_KEY_CFG_SIZE:", (int32_t *) &IC_KEY_CFG_SIZE) < 0) {
                        retval = -1;
                        goto exit_free;
                    }
				}
#endif
				else if(mp_criteria_item == X_Channel_E) {
					ptr = fbufp;
					if (parse_mp_setting_criteria_item(&ptr, "X_Channel:", (int32_t *) &X_Channel) < 0) {
						retval = -1;
						goto exit_free;
					}
				} else if(mp_criteria_item == Y_Channel_E) {
					ptr = fbufp;
                    if (parse_mp_setting_criteria_item(&ptr, "Y_Channel:", (int32_t *) &Y_Channel) < 0) {
                        retval = -1;
                        goto exit_free;
                    }
				}
#if TOUCH_KEY_NUM > 0
				else if(mp_criteria_item == Key_Channel_E) {
					ptr = fbufp;
                    if (parse_mp_setting_criteria_item(&ptr, "Key_Channel:", (int32_t *) &Key_Channel) < 0) {
                        retval = -1;
                        goto exit_free;
                    }
				}
#endif
				else if(mp_criteria_item == AIN_X_E) {
					ptr = fbufp;
					if (parse_mp_setting_ain_array(&ptr, "AIN_X:", AIN_X, IC_X_CFG_SIZE) < 0) {
					   retval = -1;
					   goto exit_free;
					}
				} else if(mp_criteria_item == AIN_Y_E) {
					ptr = fbufp;
...
#if TOUCH_KEY_NUM > 0
				else if(mp_criteria_item == AIN_KEY_E) {
					ptr = fbufp;
...
#endif
				else if(mp_criteria_item == PS_Config_Lmt_Short_Rawdata_P_E) {
					ptr = fbufp;
                    if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_Short_Rawdata_P:", PS_Config_Lmt_Short_Rawdata_P, X_Channel) < 0) {
                    	retval = -1;
                        goto exit_free;
                    }
				} else if(mp_criteria_item == PS_Config_Lmt_Short_Rawdata_N_E) {
					ptr = fbufp;
                    if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_Short_Rawdata_N:", PS_Config_Lmt_Short_Rawdata_N, X_Channel) < 0) {
                    	retval = -1;
                        goto exit_free;
                    }
				} else if(mp_criteria_item == PS_Config_Lmt_Open_Rawdata_P_E) {
					ptr = fbufp;
                    if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_Open_Rawdata_P:", PS_Config_Lmt_Open_Rawdata_P, X_Channel) < 0) {
                    	retval = -1;
                        goto exit_free;
                    }
				} else if(mp_criteria_item == PS_Config_Lmt_Open_Rawdata_N_E) {
					ptr = fbufp;
					if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_Open_Rawdata_N:", PS_Config_Lmt_Open_Rawdata_N, X_Channel) < 0) {
						retval = -1;
						goto exit_free;
					}
				} else if(mp_criteria_item == PS_Config_Lmt_FW_Rawdata_P_E) {
					ptr = fbufp;
					if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_FW_Rawdata_P:", PS_Config_Lmt_FW_Rawdata_P, X_Channel) < 0) {
						retval = -1;
						goto exit_free;
					}
				} else if(mp_criteria_item == PS_Config_Lmt_FW_Rawdata_N_E) {
					ptr = fbufp;
					if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_FW_Rawdata_N:", PS_Config_Lmt_FW_Rawdata_N, X_Channel) < 0) {
						retval = -1;
						goto exit_free;
					}
				} else if(mp_criteria_item == PS_Config_Lmt_FW_CC_P_E) {
					ptr = fbufp;
					if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_FW_CC_P:", PS_Config_Lmt_FW_CC_P, X_Channel) < 0) {
						retval = -1;
						goto exit_free;
					}
				} else if(mp_criteria_item == PS_Config_Lmt_FW_CC_N_E) {
					ptr = fbufp;
					if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_FW_CC_N:", PS_Config_Lmt_FW_CC_N, X_Channel) < 0) {
						retval = -1;
						goto exit_free;
					}
				} else if(mp_criteria_item == PS_Config_Lmt_FW_Diff_P_E) {
					ptr = fbufp;
					if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_FW_Diff_P:", PS_Config_Lmt_FW_Diff_P, X_Channel) < 0) {
						retval = -1;
						goto exit_free;
					}
				} else if(mp_criteria_item == PS_Config_Lmt_FW_Diff_N_E) {
					ptr = fbufp;
					if (parse_mp_criteria_item_array(&ptr, "PS_Config_Lmt_FW_Diff_N:", PS_Config_Lmt_FW_Diff_N, X_Channel) < 0) {
						retval = -1;
						goto exit_free;
					}
				} else if(mp_criteria_item == PS_Config_Diff_Test_Frame_E) {
					ptr = fbufp;
					if (parse_mp_setting_criteria_item(&ptr, "PS_Config_Diff_Test_Frame:", (int32_t *) &PS_Config_Diff_Test_Frame) < 0) {
						retval = -1;
						goto exit_free;
					}
				}
				...
				mp_criteria_item++;
				if (mp_criteria_item == MP_SETTIGN_CRITERIA_ITEM_LAST_E) {
					NVT_LOG("Load MP criteria and golden samples finished.\n");
					break;
				}
			}
		} else {
