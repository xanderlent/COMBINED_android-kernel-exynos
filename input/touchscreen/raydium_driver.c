...
static int raydium_gpio_configure(struct raydium_ts_data *ts, bool on)
{
	...
	if (on) {
		...
		if (gpio_is_valid(ts->rst_gpio)) {
			i32_err = gpio_request(ts->rst_gpio,
					   "raydium_rst_gpio");
			if (i32_err) {
				dev_err(&ts->client->dev,
			...
			i32_err = gpio_direction_output(ts->rst_gpio, 1);
			if (i32_err) {
				dev_err(&ts->client->dev,
					"[touch]set_direction for irq gpio failed\n");
...
int raydium_esd_check(struct raydium_ts_data *ts)
{
	...
	mutex_lock(&ts->lock);
	if (g_u8_i2c_mode == PDA2_MODE) {
		i32_ret = raydium_i2c_pda2_set_page(ts->client, ts->is_suspend
			, RAYDIUM_PDA2_PAGE_0);
		if (i32_ret < 0)
			goto exit;
		/*read esd status*/
...
#ifdef ENABLE_DUMP_DATA
int raydium_dump_data(struct raydium_ts_data *ts,
			unsigned char u8_frame, unsigned char u8_type)
{
...
{
	struct raydium_ts_data *ts =
		container_of(work, struct raydium_ts_data, dump_work.work);
	int i32_ret = 0;
	...
	/*baseline*/
	u8_data_type = 0x02;
	raydium_dump_data(ts, 1, u8_data_type);
	u8_data_type = 0x04;
	raydium_dump_data(ts, 1, u8_data_type);
	u8_data_type = 0x10;
	raydium_dump_data(ts, 10, u8_data_type);
...
static void raydium_work_handler(struct work_struct *work)
{
	...
	if ((ts->blank == FB_BLANK_VSYNC_SUSPEND ||
...
#ifdef ENABLE_DUMP_DATA
	if (g_u8_dump_flag) {
		schedule_delayed_work(&ts->dump_work, HZ);
		g_u8_dump_flag = false;
	}
#endif
#ifdef GESTURE_EN
...
#ifdef ENABLE_DUMP_DATA
	INIT_DELAYED_WORK(&ts->dump_work, raydium_dump_data_work);
#endif
