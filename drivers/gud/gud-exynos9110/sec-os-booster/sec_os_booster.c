...
{
	hrtimer_start(&mc_hrtimer,
		      ns_to_ktime((u64)LOCAL_TIMER_PERIOD * NSEC_PER_MSEC),
		      HRTIMER_MODE_REL);
}
...
static int mc_timer_init(void)
{
	...
	mc_timer_thread =
		kthread_create(kthread_worker_fn, &mc_timer_worker, "mc_timer");
...
int secos_booster_start(enum secos_boost_policy policy)
{
	...
	if (secos_booster_request_pm_qos(&secos_booster_cluster1_qos,
					 freq)) { /* KHz */
		ret = -EPERM;
		goto error;
	}
	...
	if (boost_policy == STB_PERFORMANCE) {
		/* Restore origin performance policy after spend default boost
		 * time
		 */
		if (boost_time == 0)
		...
		hrtimer_start(&timer,
			      ns_to_ktime((u64)boost_time * NSEC_PER_MSEC),
			      HRTIMER_MODE_REL);
	} else {
...
static int __init secos_booster_init(void)
{
	...
	pm_qos_add_request(&secos_booster_cluster1_qos,
