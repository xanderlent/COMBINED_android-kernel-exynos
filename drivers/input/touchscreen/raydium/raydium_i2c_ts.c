...
static int raydium_gpio_configure(struct raydium_ts_data *data, bool on)
{
	...
	if (on) {
		if (gpio_is_valid(data->pdata->irq_gpio)) {
			err =
			    gpio_request(data->pdata->irq_gpio,
					 "raydium_irq_gpio");
			if (err) {
				       ...
				       unsigned char mode)
{
	...
	for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
		retval =
		    i2c_master_send(i2c, buf, RAYDIUM_I2C_PDA_ADDRESS_LENGTH);
				...
				unsigned short length)
{
	...
	if (length == 4) {
		mode |=
		    RAYDIUM_I2C_PDA_MODE_ENABLE | RAYDIUM_I2C_PDA_2_MODE_DISABLE
	...
	if (enable) {
		...
		if (ts->blank != FB_BLANK_POWERDOWN) {
			...
			if (raydium_i2c_pda2_set_page
			    (ts->client, RAYDIUM_PDA2_PAGE_0) < 0) {
				  ...
				  unsigned char pre_palm_status)
{
	...
	// Read palm state
	ret =
	    raydium_i2c_pda2_set_page(raydium_ts->client, RAYDIUM_PDA2_PAGE_0);
	if (ret < 0) {
		goto exit_error;
	}
	ret =
	    raydium_i2c_pda2_read(raydium_ts->client,
				  RAYDIUM_PDA2_PALM_STATUS_ADDR, rbuffer, 1);
	if (ret < 0) {
		goto exit_error;
	}
...
static int raydium_set_palm_state(struct raydium_ts_data *raydium_ts,
				  unsigned char set_palm)
{
	...
	ret =
	    raydium_i2c_pda2_write(raydium_ts->client,
				   RAYDIUM_PDA2_PALM_STATUS_ADDR, wbuffer, 1);
	if (ret < 0) {
		goto exit_error;
	}
	mutex_unlock(&raydium_ts->lock);
					      ...
					      char *buf)
{
	...
	ret =
	    raydium_i2c_pda2_write(client, RAYDIUM_PDA2_HOST_CMD_ADDR, rbuffer,
				   1);
	if (ret < 0) {
		goto exit_i2c_error;
	}
	...
	do {
		...
		ret =
		    raydium_i2c_pda2_read(client, RAYDIUM_PDA2_HOST_CMD_ADDR,
					  rbuffer, 1);
		if (ret < 0) {
			goto exit_i2c_error;
		}
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	if (u8_i2c_mode == PDA2_MODE) {
		...
		*(unsigned long *)rbuffer = (RAYDIUM_I2C_PDA_MODE_ENABLE << 24) | ((RAYDIUM_CHECK_I2C_CMD & (~MASK_8BIT)) >> 8);	//using byte mode to read 4 bytes
		...
		ret =
		    raydium_i2c_pda2_write(client, RAYDIUM_PDA2_PDA_CFG_ADDR,
					   rbuffer, 4);
		if (ret < 0) {
			goto exit_i2c_error;
		}
		ret =
		    raydium_i2c_pda2_set_page(client, RAYDIUM_PDA2_ENABLE_PDA);
		if (ret < 0) {
			goto exit_i2c_error;
		}
		ret =
		...
		ret =
		    raydium_i2c_pda_read(client, RAYDIUM_CHECK_I2C_CMD, rbuffer,
					 4);
		if (ret < 0) {
			goto exit_i2c_error;
		}
					    ...
					    char *arguments, size_t count)
{
	...
	while (uc_fw_amount--) {
		...
		ret = request_firmware(&fw, token, &client->dev);
		if (ret) {
			pr_err
			    ("[touch]error occured while requesting firmware %s.\n",
			     token);
			goto exit_upgrade;
		}

		if (fw->size > RAYDIUM_FW_MAX_SIZE - 4) {
			pr_err("[touch] firmware size exceeds buffer\n");
			ret = -ENOMEM;
			goto exit_upgrade;
		}

		if (fw->size < ul_fw_size) {
			pr_err("[touch] firmware smaller than expected\n");
			ret = -EINVAL;
			goto exit_upgrade;
		}

		memcpy(firmware_data, fw->data, ul_fw_size);
		release_firmware(fw);
		...
		while (ul_write_offset < ul_fw_size) {
			if ((ul_write_offset + MAX_WRITE_PACKET_SIZE) <
				...
				us_write_length =
				    (unsigned short)(ul_fw_size -
						     ul_write_offset);
			}
			ret =
			    raydium_i2c_pda_write(client,
						  (ul_fw_offset +
						   ul_write_offset),
						  (firmware_data +
						   ul_write_offset),
					    ...
					    const char *buf, size_t count)
{
	...
	if (temp_buf && data_count) {
		...
		mutex_lock(&ts->lock);
		result =
		    raydium_i2c_pda_write(client, g_ul_addr, w_data,
					  g_ui_length);
		mutex_unlock(&ts->lock);
		if (result < 0) {
			ret = result;
					     ...
					     const char *buf, size_t count)
{
	...
	if (temp_buf && data_count) {
		...
		mutex_lock(&ts->lock);
		result =
		    raydium_i2c_pda2_write(client, g_uc_addr, w_data,
					   g_ui_length);
		mutex_unlock(&ts->lock);
		if (result < 0) {
			ret = result;
					  ...
					  const char *buf, size_t count)
{
	...
	result =
	    raydium_i2c_pda2_write(client, RAYDIUM_PDA2_HOST_CMD_ADDR, w_data,
				   1);
	if (result < 0) {
		mutex_unlock(&ts->lock);
		ret = result;
...
static int raydium_hw_reset_fun(struct i2c_client *client)
{
	...
	while (retry != 0 && g_uc_raydium_flag != RAYDIUM_INTERRUPT_FLAG) {
		...
		retry--;
		printk(KERN_INFO
		       "[touch]wait software reset finish => confirm interrupt_flag result %d\n",
		       retry);
	}
				       ...
				       const char *buf, size_t count)
{
	...
	w_data[0] = palm_area;
	ret =
	    raydium_i2c_pda2_write(client, RAYDIUM_PDA2_PALM_AREA_ADDR, w_data,
				   1);
	if (ret < 0) {
		mutex_unlock(&ts->lock);
		goto exit_error;
					...
					const char *buf, size_t count)
{
	...
	case 1:		/* Enable Touch lock */
		...
		mutex_lock(&ts->lock);
		ret =
		    raydium_i2c_pda2_set_page(ts->client, RAYDIUM_PDA2_PAGE_0);
		if (ret < 0) {
					     ...
					     char *buf)
{
	...
	ret =
	    raydium_i2c_pda2_read(client, RAYDIUM_PDA2_FW_VERSION_ADDR, rbuffer,
				  4);
	if (ret < 0) {
		goto exit_i2c_error;
	}
	...
	fw_version =
	    (rbuffer[0] << 24) | (rbuffer[1] << 16) | (rbuffer[2] << 8) |
	    rbuffer[3];
	printk(KERN_ERR "[touch]Raydium IC FW version is 0x%x\n", fw_version);

#if defined(UPGRADE_FROM_IMAGE)
	image_version =
	    (RadFWImage[0xF21] << 24) | (RadFWImage[0xF22] << 16) |
	    (RadFWImage[0xF23] << 8) | RadFWImage[0xF24];
	printk(KERN_ERR "[touch]Raydium Image FW version is 0x%x\n",
	       image_version);
#endif // UPGRADE_FROM_IMAGE
						...
						char *buf)
{
	...
	ret =
	    raydium_i2c_pda2_read(client, RAYDIUM_PDA2_PANEL_VERSION_ADDR,
				  rbuffer, 8);
	if (ret < 0) {
		goto exit_i2c_error;
	}
...
static ssize_t fw_check_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	int count = 0;
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	...
	if (ts->fw_version == 0) {
		...
		count +=
		    sprintf(buf + count, "Pass, (fw_ver : 0x%x)\n",
			    ts->fw_version);
	}
	return count;
}

static unsigned char i2c_burst_read(struct i2c_client *client, int int_addr,
...
static unsigned char i2c_burst_write(struct i2c_client *client, int int_addr,
				     unsigned short bWriteLen,
				     unsigned char *bValue)
{
	...
	while (u16_len < bWriteLen) {
		ret =
		    raydium_i2c_pda_write(client, int_addr, bValue + u16_len,
					  u8_w_len);
		if (ret < 0) {
			return result;
		}
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	unsigned int writeIndex = 0, i2cWriteLen = 0, i2cReadLength =
	    0, readLength = 0;
	int addr = 0x20000000;
...
RETRY_I2C:
	while (i16_time_out > 0 && u8_is_pass != 1) {
		...
		while (writeIndex < u32_test_length) {
			if (i2c_burst_write
			    (client, addr + writeIndex, i2cWriteLen,
			     wData) == 0) {
				printk(KERN_ERR
		...
		while (readLength < u32_test_length) {
			if (i2c_burst_read
			    (client, addr + readLength, i2cReadLength,
			     readData) == 0) {
				i16_time_out--;
				printk(KERN_ERR
			...
			for (writeIndex = 0; writeIndex < i2cReadLength;
			     writeIndex++) {
				if (wData[writeIndex] != readData[writeIndex]) {
					printk(KERN_ERR
					       "[touch]i2c_test Read NG [%d], W=0x%x,R=0x%x\r\n",
					       writeIndex, wData[writeIndex],
					       readData[writeIndex]);
					i16_time_out--;
 ...
 */
static DEVICE_ATTR(raydium_check_i2c, 0644, raydium_check_i2c_show, NULL);
...
static DEVICE_ATTR(raydium_upgrade_firmware, 0644,
		   NULL, raydium_upgrade_firmware_store);
...
static DEVICE_ATTR(raydium_i2c_pda_access, 0644,
		   raydium_i2c_pda_access_show, raydium_i2c_pda_access_store);
...
static DEVICE_ATTR(raydium_i2c_pda2_access, 0644,
		   raydium_i2c_pda2_access_show, raydium_i2c_pda2_access_store);
 ...
 */
static DEVICE_ATTR(raydium_flag, 0644, raydium_flag_show, raydium_flag_store);

/* Read interrupt flag cmd (R)
 *  example:cat raydium_int_flag
 */
static DEVICE_ATTR(raydium_int_flag, 0644,
		   raydium_int_flag_show, raydium_int_flag_store);

/* Read selftest flag cmd (R)
 *  example:cat raydium_int_flag
 */
static DEVICE_ATTR(raydium_selftest_flag, 0644,
		   raydium_selftest_flag_show, raydium_selftest_flag_store);
...
static DEVICE_ATTR(raydium_hw_reset, 0644, raydium_hw_reset_show, NULL);
static DEVICE_ATTR(raydium_wait_ft_int, 0644,
		   raydium_wait_ft_int_show, raydium_wait_ft_int_store);
static DEVICE_ATTR(raydium_palm_status, 0644,
		   raydium_palm_status_show, raydium_palm_status_store);

static DEVICE_ATTR(raydium_palm_area, 0644, NULL, raydium_palm_area_store);
static DEVICE_ATTR(raydium_i2c_test, 0644, raydium_i2c_test_show, NULL);
...
static int raydium_read_touchdata(struct raydium_ts_data *data)
{
	...
	if (data->blank == FB_BLANK_VSYNC_SUSPEND
		...
		if (g_u8_unlockflag == true) {
			...
			ret =
			    raydium_i2c_pda2_set_page(data->client,
						      RAYDIUM_PDA2_PAGE_0);
			if (ret < 0) {
				dev_err(&data->client->dev,
			...
			if (buf[0] == 1) {
				...
				input_mt_report_slot_state(data->input_dev,
							   MT_TOOL_FINGER, 0);
				input_mt_report_pointer_emulation(data->
								  input_dev,
								  false);
				input_sync(data->input_dev);
		...
		if (u8_seq_no == tp_status[POS_SEQ]) {	//report not updated
			mutex_unlock(&data->lock);
		...
		for (i = 0; i < u8_points_amount; i++) {
			...
			// New coming
			if (j == MAX_TOUCH_NUM) {
				for (j = MAX_TOUCH_NUM; j < (MAX_TOUCH_NUM * 2);
				     j++) {
					if (!gst_slot_status[j].need_update) {
						gst_slot_status[j].
						    occupied_pt_id =
						    u8_point_id;
						gst_slot_status[j].need_update =
						    1;
						gst_slot_status[j].
						    pt_report_offset = i;
						printk(KERN_INFO
		...
		for (i = 0; i < MAX_TOUCH_NUM; i++) {
			if (gst_slot_status[i].need_update) {
				offset =
				    gst_slot_status[i].pt_report_offset *
				    LENGTH_PT;
				s16_x =
				...
				input_report_abs(data->input_dev,
						 ABS_MT_PRESSURE, s16_pressure);
				input_report_abs(data->input_dev,
						 ABS_MT_TOUCH_MAJOR, max(s16_wx,
									 s16_wy));
				input_report_abs(data->input_dev,
						 ABS_MT_TOUCH_MINOR, min(s16_wx,
									 s16_wy));
	...
	if (raydium_ts->blank == FB_BLANK_UNBLANK)	//when display on can use palm to suspend
	{
		...
		if (palm_status == RAYDIUM_PALM_MODE_ENABLE) {
			if (raydium_ts->is_palm == 0) {
				// release all touches
				for (i = 0;
				     i < raydium_ts->pdata->num_max_touches;
				     i++) {
					input_mt_slot(raydium_ts->input_dev, i);
					input_mt_report_slot_state(raydium_ts->
								   input_dev,
								   MT_TOOL_FINGER,
								   false);
				}

				input_mt_report_pointer_emulation(raydium_ts->
								  input_dev,
								  false);
...
static irqreturn_t raydium_ts_interrupt(int irq, void *dev_id)
{
	...
	//For bootloader wrt/erase flash and software reset interrupt
		...
		if (!work_pending(&raydium_ts->work)) {
			result =
			    queue_work(raydium_ts->workqueue,
				       &raydium_ts->work);
				...
				if (raydium_ts->blank == FB_BLANK_POWERDOWN) {
					...
					if (raydium_i2c_pda2_set_page
						...
						disable_irq_nosync(raydium_ts->
								   irq);
						raydium_ts->irq_enabled = false;
						dev_err(&raydium_ts->client->
							dev,
							"[touch]%s: failed to set page\n",
							__func__);
					}
					mutex_unlock(&raydium_ts->lock);
				}
...
static int raydium_check_i2c_ready(struct raydium_ts_data *raydium_ts)
{
	...
	if (u8_i2c_mode == PDA2_MODE) {
		...
		*(unsigned long *)temp_buf = (RAYDIUM_I2C_PDA_MODE_ENABLE << 24) | ((RAYDIUM_CHECK_I2C_CMD & (~MASK_8BIT)) >> 8);	//using byte mode to read 4 bytes
		ret =
		    raydium_i2c_pda2_write(raydium_ts->client,
					   RAYDIUM_PDA2_PDA_CFG_ADDR, temp_buf,
					   4);
		if (ret < 0) {
			goto exit_error;
		}
		...
		ret =
		    raydium_i2c_pda_read(raydium_ts->client,
					 RAYDIUM_CHECK_I2C_CMD, temp_buf, 4);
		if (ret < 0) {
			goto exit_error;
		}
	}
...
static void raydium_ts_close(struct input_dev *input_dev)
{
	...
	wbuffer[0] = RAYDIUM_HOST_CMD_PWR_SLEEP;	//fw enter sleep mode
	ret =
	    raydium_i2c_pda2_write(ts->client, RAYDIUM_PDA2_HOST_CMD_ADDR,
				   wbuffer, 1);
	if (ret < 0) {
		printk(KERN_ERR "[touch]ret:%d\n", ret);
		goto exit_i2c_error;
			    ...
			    const struct i2c_device_id *id)
{
	...
	raydium_ts =
	    devm_kzalloc(&client->dev, sizeof(struct raydium_ts_data),
			 GFP_KERNEL);
	if (!raydium_ts) {
		dev_err(&client->dev,
	...
	dev_err(&client->dev, "[touch]client irq : %d, pdata flags : %d\n", raydium_ts->irq, pdata->irqflags);	//108

	raydium_ts->irq = gpio_to_irq(raydium_ts->pdata->irq_gpio);
	ret = request_threaded_irq(raydium_ts->irq, NULL, raydium_ts_interrupt,
				   pdata->
	...
	raydium_ts->irq_desc = irq_to_desc(raydium_ts->irq);
...
static int raydium_ts_remove(struct i2c_client *client)
{
	...
	free_irq(raydium_ts->irq, raydium_ts);
