...
static void dsim_set_hw_deskew(u32 id, u32 en)
{
	...
	if (en) {
		...
		dsim_reg_set_deskew_hw(id, hw_interval, 1);
		...
		struct dsim_clks *clks)
{
	...
	dsim_reg_set_config_options(id, lcd_info->data_lane -1, 1,
			1, DSIM_PIXEL_FORMAT_RGB24, 0);
...
void dsim_reg_preinit(u32 id)
{
	...
	lcd_info.dphy_pms.m = 0x0EC;
	lcd_info.dphy_pms.s = 2;
	lcd_info.dphy_pms.k = 0x4EC5;
