...
static irqreturn_t dw_mci_interrupt(int irq, void *dev_id)
{
	...
	if (host->slot->mmc->card &&
		...
		pr_debug("*** %s: cmdq intr: 0x%08x\n",
				...
				pending);
		if (reg) {
			if (reg & (SDMMC_INT_RCRC | SDMMC_INT_RESP_ERR |
				   SDMMC_INT_HLE))
				cmd_error = -EILSEQ;
				...
				cmd_error = 0;
			if (reg & (SDMMC_INT_EBE | SDMMC_INT_DCRC |
				   SDMMC_INT_SBE))
				data_error = -EILSEQ;
...
static void dw_mci_timeout_timer(struct timer_list *t)
{
	...
	if (host && host->mrq) {
		...
		dw_mci_fifo_reset(host->dev, host);
		int_mask = mci_readl(host, INTMASK);
		if (~int_mask & (SDMMC_INT_CMD_DONE | SDMMC_INT_DATA_OVER |
				 DW_MCI_ERROR_FLAGS)) {
			if (host->use_dma)
				int_mask |= (SDMMC_INT_CMD_DONE |
					     SDMMC_INT_DATA_OVER |
					     DW_MCI_ERROR_FLAGS);
			else
				int_mask |= (SDMMC_INT_CMD_DONE |
...
static void dw_mci_cmdq_enable(struct mmc_host *mmc)
{
	...
	dw_mci_set_timeout(host,
		(SDMMC_DATA_TMOUT_MAX_EXT_CNT << SDMMC_DATA_TMOUT_EXT_SHIFT) |
			SDMMC_RESP_TMOUT);
	host->cqe_on = true;
}
...
static int dw_mci_cmdq_crypto_engine_cfg(struct mmc_host *mmc,
					 void *desc,
					 struct mmc_data *data,
					 struct page *page,
					 int page_index,
					 ...
					 bool cmdq_enabled)
{
	...
	return drv_data->crypto_engine_cfg(host,
					   desc,
					   data,
					   page,
					   page_index,
...
static int dw_mci_cmdq_crypto_engine_clear(struct mmc_host *mmc,
					   void *desc,
					   struct mmc_data *data,
					   bool cmdq_enabled)
{
...
int dw_mci_probe(struct dw_mci *host, struct platform_device *pdev)
{
	...
	if (IS_ERR(host->ciu_gate))
		dev_dbg(host->dev,
			"not used ciu gate clock or not available\n");
	...
	host->sd_card_det_workqueue = alloc_workqueue("sd-card-det-wq",
						      WQ_MEM_RECLAIM |
						      WQ_UNBOUND |
						      WQ_HIGHPRI, 1);
