...
static int exynos_usbdrd_pipe3_bc_check(struct exynos_usbdrd_phy *phy_drd)
{
	return 0;
}

static int exynos_usbdrd_utmi_bc_check(struct exynos_usbdrd_phy *phy_drd)
{
...
static int exynos_usbdrd_phy_bc_check(struct phy *phy)
{
	struct phy_usb_instance *inst = phy_get_drvdata(phy);
	struct exynos_usbdrd_phy *phy_drd = to_usbdrd_phy(inst);

	dev_info(phy_drd->dev, "UIMI or PIPE3 specific BC check\n");

	/* UTMI or PIPE3 specific BC check */
	return inst->phy_cfg->phy_bc_check(phy_drd);
}
...
static struct phy_ops exynos_usbdrd_phy_ops = {
	...
	.bc_check	= exynos_usbdrd_phy_bc_check,
	.owner		= THIS_MODULE,
};
...
static const struct exynos_usbdrd_phy_config phy_cfg_exynos[] = {
	{
		...
		.phy_bc_check	= exynos_usbdrd_utmi_bc_check,
	},
	{
		...
		.phy_bc_check	= exynos_usbdrd_pipe3_bc_check,
	},
};
