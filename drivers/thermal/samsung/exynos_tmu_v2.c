...
static int exynos9110_tmu_initialize(struct platform_device *pdev)
{
	struct exynos_tmu_data *data = platform_get_drvdata(pdev);
...
static void exynos9110_tmu_control(struct platform_device *pdev, bool on)
{
	struct exynos_tmu_data *data = platform_get_drvdata(pdev);
	unsigned int trim, ctrl, con1, avgc;
	unsigned int t_buf_vref_sel, t_buf_slope_sel, avg_mode;
	unsigned int counter_value;
	unsigned int t_normal_mode;
...
static void exynos9110_tmu_set_emulation(struct exynos_tmu_data *data,
					 int temp)
{
	unsigned int val;
	u32 emul_con;
...
static int exynos9110_tmu_read(struct exynos_tmu_data *data)
{
	int temp = 0;
	...
	if (data->sensor_info[0].sensor_num < 2) {
		reg_offset = 0;
		bit_offset = EXYNOS_TMU_TEMP_SHIFT * data->sensor_info[0].sensor_num;
	} else {
		reg_offset = ((data->sensor_info[0].sensor_num - 2) / 3 + 1) * 4;
		bit_offset = EXYNOS_TMU_TEMP_SHIFT * ((data->sensor_info[0].sensor_num - 2) % 3);
	}

	temp_code = (readl(data->base + EXYNOS_TMU_REG_CURRENT_TEMP1_0 + reg_offset)
			>> bit_offset) & EXYNOS_TMU_TEMP_MASK;
	temp = code_to_temp_with_sensorinfo(data, temp_code, &data->sensor_info[0]);
#endif
...
static void exynos9110_tmu_clear_irqs(struct exynos_tmu_data *data)
{
...
{
	if (of_device_is_compatible(np, "samsung,exynos9110-tmu"))
		return SOC_ARCH_EXYNOS9110;
...
static int exynos_map_dt_data(struct platform_device *pdev)
{
	...
	switch (data->soc) {
	case SOC_ARCH_EXYNOS9110:
		data->tmu_initialize = exynos9110_tmu_initialize;
		data->tmu_control = exynos9110_tmu_control;
		data->tmu_read = exynos9110_tmu_read;
		data->tmu_set_emulation = exynos9110_tmu_set_emulation;
		data->tmu_clear_irqs = exynos9110_tmu_clear_irqs;
		data->ntrip = 8;
		break;
	default:
...
static int exynos_tmu_parse_ect(struct exynos_tmu_data *data)
{
	...
	if (strncasecmp(tz->tzp->governor_name, "power_allocator",
		...
		for (i = 0; i < function->num_of_range; ++i) {
			...
			tz->ops->set_tbp_value(tz, i, freq);
