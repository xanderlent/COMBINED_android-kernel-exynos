 ...
 */
static unsigned long get_level(struct exynos_cpufreq_cooling_device *cpufreq_dev,
			       unsigned int freq)
{
	unsigned long level;
...
{
	struct exynos_cpufreq_cooling_device *cpufreq_dev;
				    ...
				    unsigned long event, void *data)
{
	...
	list_for_each_entry(cpufreq_dev, &cpufreq_dev_list, node) {
		...
		if (policy->max > clipped_freq) {
			...
			dbg_snapshot_thermal(NULL, 0, cpufreq_dev->cool_dev->type, clipped_freq);
		}
		break;
	}
	mutex_unlock(&cooling_list_lock);
 ...
 */
static int build_dyn_power_table(struct exynos_cpufreq_cooling_device *cpufreq_device,
				 u32 capacitance)
{
...
static int build_static_power_table(struct exynos_cpufreq_cooling_device *cpufreq_device, char *tz_name)
{
	int i, j;
...
static int lookup_static_power(struct exynos_cpufreq_cooling_device *cpufreq_device,
...
static u32 cpu_freq_to_power(struct exynos_cpufreq_cooling_device *cpufreq_device,
			     u32 freq)
{
	int i;
...
static u32 cpu_power_to_freq(struct exynos_cpufreq_cooling_device *cpufreq_device,
			     u32 power)
{
	int i;
	struct power_table *pt = cpufreq_device->dyn_power_table;
 ...
 * @cpufreq_device:	&struct exynos_cpufreq_cooling_device for this cpu
 * @cpu:	cpu number
 ...
 */
static u32 get_load(struct exynos_cpufreq_cooling_device *cpufreq_device, int cpu,
		    int cpu_idx)
{
	u32 load;
 ...
 * @cpufreq_device:	struct &exynos_cpufreq_cooling_device for this cpu cdev
 ...
 */
static int get_static_power(struct exynos_cpufreq_cooling_device *cpufreq_device,
 ...
 * @cpufreq_device:	&exynos_cpufreq_cooling_device for this cdev
 * @freq:	current frequency
 *
 * Return: the dynamic power consumed by the cpus described by
 * @cpufreq_device.
 */
static u32 get_dynamic_power(struct exynos_cpufreq_cooling_device *cpufreq_device,
			     unsigned long freq)
{
	u32 raw_cpu_power;
...
static int cpufreq_get_max_state(struct thermal_cooling_device *cdev,
				 unsigned long *state)
{
	struct exynos_cpufreq_cooling_device *cpufreq_device = cdev->devdata;
				       ...
				       u32 *power)
{
	...
	for_each_cpu(cpu, &cpufreq_device->allowed_cpus) {
		...
		total_load += load;
		if (trace_thermal_exynos_power_cpu_limit_enabled() && load_cpu)
			load_cpu[i] = load;
	...
	if (load_cpu) {
		trace_thermal_exynos_power_cpu_get_power(
			       ...
			       unsigned long *state)
{
	...
	*state = cpufreq_cooling_get_level(cpu, target_freq);
	...
	trace_thermal_exynos_power_cpu_limit(&cpufreq_device->allowed_cpus,
			...
			char *tz_name)
{
	...
	if (capacitance) {
		...
		ret = build_static_power_table(cpufreq_dev, tz_name);
		if (ret) {
			cool_dev = ERR_PTR(ret);
			goto free_table;
		}
