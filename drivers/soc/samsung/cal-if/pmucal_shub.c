...
int pmucal_shub_init(void)
{
	...
	if (!pmucal_shub_list.init) {
		pr_err("%s there is no sequence element for shub-init.\n",
				PMUCAL_PREFIX);
		return -ENOENT;
	}

	ret = pmucal_rae_handle_seq(pmucal_shub_list.init,
				pmucal_shub_list.num_init);
	if (ret) {
		pr_err("%s %s: error on handling shub-init sequence.\n",
				PMUCAL_PREFIX, __func__);
		return ret;
	}
...
/**
 *  pmucal_shub_cpu_status - get shub cpu status.
 *		        exposed to PWRCAL interface.

 *  Returns 1 when shub is run, 0 when shub is reset.
 *  Otherwise, negative error code.
 */
int pmucal_shub_cpu_status(void)
{
	pr_info("%s%s()\n", PMUCAL_PREFIX, __func__);

	if (!pmucal_shub_list.cpu_status) {
		pr_err("%s there is no sequence element for shub-status.\n",
				PMUCAL_PREFIX);
		return -ENOENT;
	}
	pmucal_rae_handle_seq(pmucal_shub_list.cpu_status,
				pmucal_shub_list.num_cpu_status);
	pr_err("shub_cpu_status: 0x%X\n", pmucal_shub_list.cpu_status->value);
	return pmucal_shub_list.cpu_status->value;
}

/**
 *  pmucal_shub_status - get shub status.
 *		        exposed to PWRCAL interface.

 *  Returns 1 when the shub is initialized, 0 when not.
 *  Otherwise, negative error code.
 */
int pmucal_shub_status(void)
{
	pr_info("%s%s()\n", PMUCAL_PREFIX, __func__);
	...
	if (pmucal_shub_list.status->value == 0x5)
		return 1;
	else
		return 0;
}
...
/**
 *  pmucal_shub_reset_release - reset_release shub.
 *		        exposed to PWRCAL interface.
 *
 *  Returns 0 on success. Otherwise, negative error code.
 */
...
int __init pmucal_shub_initialize(void)
{
	...
	ret = pmucal_rae_phy2virt(pmucal_shub_list.cpu_status,
				pmucal_shub_list.num_cpu_status);
	if (ret) {
		pr_err("%s %s: error on PA2VA conversion. aborting status...\n",
				PMUCAL_PREFIX, __func__);
