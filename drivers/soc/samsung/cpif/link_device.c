				...
				unsigned long arg)
{
	...
	if (err == CP_ALREADY_DUMP_MODE) {
		mif_info("CP already dump mode, return success\n");
		err = CP_NO_ERROR;
	}
...
#if defined(CONFIG_SOC_EXYNOS9110)
	mif_info("Parsing dvfs_mif frequency table...\n");
	err = parse_ect(mld, "dvfs_mif");
	if (err < 0)
		mif_err("Can't get dvfs_mif frequency table!!!!!\n");

	mif_info("Parsing dvfs_cpucl0 frequency table...\n");
	err = parse_ect(mld, "dvfs_cpucl0");
	if (err < 0)
		mif_err("Can't get dvfs_cpucl0 frequency table!!!!!\n");

	mif_info("Parsing dvfs_int frequency table...\n");
	err = parse_ect(mld, "dvfs_int");
	if (err < 0)
		mif_err("Can't get dvfs_int frequency table!!!!!\n");
#else
