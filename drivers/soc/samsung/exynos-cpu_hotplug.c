...
static struct cpumask create_cpumask(void)
{
	...
	online_cpu_min = min(pm_qos_request(PM_QOS_CPU_ONLINE_MIN), (int)nr_cpu_ids);
	online_cpu_max = min(pm_qos_request(PM_QOS_CPU_ONLINE_MAX), (int)nr_cpu_ids);
		...
		unsigned int list)
{
	...
	ret = blocking_notifier_chain_register(&exynos_cpuhotplug_notifier_list, nb);
		...
		unsigned int list)
{
	...
	ret = blocking_notifier_chain_unregister(&exynos_cpuhotplug_notifier_list, nb);
...
out:
	...
	blocking_notifier_call_chain(&exynos_cpuhotplug_notifier_list, UPDATE_ONLINE_CPU, NULL);
