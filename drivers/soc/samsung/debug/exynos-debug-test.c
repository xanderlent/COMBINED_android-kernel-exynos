...
static void simulate_WP(char *arg)
{
	...
	if (exynos_debug_desc.ps_hold_control_offset == UINT_MAX) {
		dev_err(exynos_debug_desc.dev, "no such ps_hold_control_offset\n");
		return;
	}
...
static int exynos_debug_test_parsing_dt(struct device_node *np)
{
	...
	if (ret) {
		...
		exynos_debug_desc.ps_hold_control_offset = UINT_MAX;
	}
